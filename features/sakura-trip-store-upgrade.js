...
(function upgradeSakuraTripStore(){
  ...
  const clean=core.clean,compact=core.compact;
  ...
  function upgradeExisting(trip){
    const copy=clone(trip);copy.days=(copy.days||[]).map(day=>({...day,items:core.assignStableIds(day)}));return copy;
  }
  function rawLoad(){return base.loadTrips().map(upgradeExisting)}
  function loadTrips(){return rawLoad()}
  function saveTrips(trips){return base.saveTrips((trips||[]).map(upgradeExisting))}
  function getTrips(){return loadTrips().map(clone)}
  function selectedTrip(){const trips=loadTrips(),wanted=localStorage.getItem(base.keys.ACTIVE_TRIP_KEY);return trips.find(t=>t.id===wanted)||trips[0]||null}
  ...
  function carryIds(existing,incoming){
    const prev=upgradeExisting(existing||{items:[]}),next=clone(incoming||{items:[]});
    const bySig=new Map((prev.items||[]).map(i=>[core.baseItemSignature(i,prev.date||next.date),i]));
    ...
    next.items=core.assignStableIds(next);
    return next;
  }
  function mergeDay(existing,incoming){
    const next=carryIds(existing,incoming),map=new Map();
    for(const item of core.assignStableIds(existing||{}))map.set(itemKey(item,existing.date),item);
  ...
  function preserveTripIds(existing,incoming){
    const byDate=new Map((existing?.days||[]).map(d=>[d.date,d]));
    return {...incoming,days:(incoming.days||[]).map(d=>byDate.has(d.date)?carryIds(byDate.get(d.date),d):({...d,items:core.assignStableIds(d)}))};
  }
  ...
  function applyImport(imported,mode='add'){
    ...
    if(imported.days.length===1&&match.days.some(d=>d.date===imported.days[0].date)){
      ...
      const byDate=new Map(next.days.map(d=>[d.date,d]));for(const d of imported.days)byDate.set(d.date,byDate.has(d.date)?mergeDay(byDate.get(d.date),d):d);trips[index]=normalizeTrip({...next,...imported,id:match.id,days:[...byDate.values()],source:'import'});
    }else{
      trips[index]=normalizeTrip({...preserveTripIds(match,imported),id:match.id,source:'import'});
    }
  ...
  try{
    const before=base.loadTrips(),after=before.map(upgradeExisting);const changed=JSON.stringify(before)!==JSON.stringify(after);if(changed)base.saveTrips(after);
  }catch(error){console.warn('Sakura trip stable-ID migration skipped.',error)}
}());
