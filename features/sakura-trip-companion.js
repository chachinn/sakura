/* Sakura Trip Companion loader v27 */
(function loadSakuraTripCompanion(){
  'use strict';
  if(window.__sakuraTripCompanionLoadingV27)return;
  window.__sakuraTripCompanionLoadingV27=true;
  ...
  const assets=[
    ...
    ['./features/sakura-trip-core.js?v=2','sakura-trip-core'],
    ...
    ['./features/sakura-trip-quick-help-jump.js?v=1','sakura-trip-quick-help-jump'],
    ...
    ['./features/sakura-camera-japanese-v2.js?v=2','sakura-camera-japanese-v2'],
    ...
    ['./features/sakura-trip-transit-bridge.js?v=3','sakura-trip-transit-bridge'],
    ...
    ['./features/sakura-trip-rail-runtime-guard.js?v=2','sakura-trip-rail-runtime-guard']
  ];
  ...
  const ready=key=>
    ...
    (key==='sakura-trip-core'&&window.SakuraTripCore?.version>=2)||
    (key==='sakura-trip-store-upgrade'&&window.SakuraTripStore?.version>=2)||
    (key==='sakura-trip-ui'&&window.SakuraTripCompanion?.version>=2)||
    (key==='sakura-trip-quick-help-jump'&&window.SakuraTripQuickHelpJump?.version>=1)||
    ...
    (key==='sakura-camera-japanese-v2'&&window.SakuraCameraJapanese?.version>=3)||
    ...
    (key==='sakura-trip-transit-bridge'&&window.SakuraTripTransitBridge?.version>=3)||
    (key==='sakura-trip-pinned-rail'&&window.SakuraTripPinnedRail?.version>=2)||
    (key==='sakura-trip-rail-runtime-guard'&&window.SakuraTripRailRuntimeGuard?.version>=2)||
  ...
  const load=(src,key)=>new Promise((resolve,reject)=>{
    ...
    if(existing){
      if(existing.dataset.loaded==='1'){
        if(ready(key)){if(key==='sakura-sakutalk-v3')refreshSakuTalkV3();resolve();return}
        existing.remove();
      }else{
        existing.addEventListener('load',()=>{if(key==='sakura-sakutalk-v3')refreshSakuTalkV3();if(ready(key))resolve();else reject(new Error(`${src} loaded without the required runtime version.`))},{once:true});
        existing.addEventListener('error',()=>reject(new Error(`Could not load ${src}`)),{once:true});return;
      }
    }
    ...
    script.onload=()=>{script.dataset.loaded='1';if(!ready(key)){reject(new Error(`${src} loaded without the required runtime version.`));return}if(key==='sakura-sakutalk-v3'){refreshSakuTalkV3();setTimeout(refreshSakuTalkV3,0)}resolve()};
  ...
  (async()=>{
    try{
      ...
      window.SakuraTripCompanion?.ensureLauncher?.();window.SakuraTripPublicDefault?.ensureEmptyLauncher?.();window.SakuraTripLiveTools?.decorate?.();window.SakuraTripCompanionPolish?.decorate?.();window.SakuraTripCompanionStabilizer?.decorate?.();window.SakuraTripQuickHelpJump?.decorate?.();refreshSakuTalkV3();if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',refreshSakuTalkV3,{once:true});
    }catch(error){console.warn('Sakura Trip Companion could not fully load. Normal Travel Mode remains available.',error)}finally{window.__sakuraTripCompanionLoadingV27=false}
  })();
}());
