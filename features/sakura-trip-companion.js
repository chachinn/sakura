/* Sakura Trip Companion loader v18 */
(function loadSakuraTripCompanion(){
  'use strict';
  if(window.__sakuraTripCompanionLoadingV18)return;
  window.__sakuraTripCompanionLoadingV18=true;
  ...
  const assets=[
    ...
    ['./features/sakura-trip-source-persistence.js?v=2','sakura-trip-source-persistence'],
    ...
    ['./features/sakura-trip-camera-nav.js?v=1','sakura-trip-camera-nav'],
    ['./features/sakura-trip-management.js?v=2','sakura-trip-management'],
    ...
    ['./features/sakura-trip-experience.js?v=1','sakura-trip-experience']
  ];
  ...
  const load=(src,key)=>new Promise((resolve,reject)=>{
    if((key==='sakura-sakutalk-v3'&&window.SakuraInterpreter?.version>=3)||
       ...
       (key==='sakura-trip-source-persistence'&&window.SakuraTripSourcePersistence?.version>=2)||
       (key==='sakura-trip-file-sync'&&window.SakuraTripFileSync?.version>=2)||
       ...
       (key==='sakura-trip-camera-nav'&&window.SakuraTripCameraNav?.version>=1)||
       (key==='sakura-trip-management'&&window.SakuraTripManagement?.version>=2)||
       ...
       (key==='sakura-trip-experience'&&window.SakuraTripExperience?.version>=1)){resolve();return}

    const existing=document.querySelector(`script[data-${key}]`);
    if(existing){
      if(existing.dataset.loaded==='1'){if(key==='sakura-sakutalk-v3')refreshSakuTalkV3();resolve();return}
