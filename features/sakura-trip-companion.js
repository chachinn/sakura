/* Sakura Trip Companion loader v24 */
(function loadSakuraTripCompanion(){
  'use strict';
  if(window.__sakuraTripCompanionLoadingV24)return;
  window.__sakuraTripCompanionLoadingV24=true;
  ...
  const ready=key=>
    ...
    (key==='sakura-trip-rail-runtime-guard'&&window.SakuraTripRailRuntimeGuard?.version>=1)||
  ...
  (async()=>{
    ...
    }catch(error){console.warn('Sakura Trip Companion could not fully load. Normal Travel Mode remains available.',error)}finally{window.__sakuraTripCompanionLoadingV24=false}
  })();
}());
