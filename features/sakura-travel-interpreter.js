...
let result=null,busy=false,recognition=null;
...
function status(t){const n=document.querySelector('[data-ti-status]');if(n)n.textContent=t}
...
function close(){$('sakura-interpreter').hidden=true;document.documentElement.style.overflow='';document.body.style.overflow='';try{speechSynthesis.cancel()}catch{}}
...
try{recognition?.abort?.();recognition=new SR();recognition.lang='en-US';recognition.interimResults=false;recognition.maxAlternatives=1;recognition.onstart=()=>status('Listening… speak naturally.');recognition.onresult=e=>{const t=e.results?.[0]?.[0]?.transcript||'';if(t)$('sakura-ti-input').value=t;status(t?'Got it. Check the sentence, then translate.':'I could not hear that clearly.')};recognition.onerror=e=>status(e.error==='not-allowed'?'Microphone permission is needed for voice input.':'Voice input did not work. You can type instead.');recognition.start()}catch{status('Voice input could not start. You can type instead.')}
}
...
if(t.closest('[data-ti-clear]')){$('sakura-ti-input').value='';$('sakura-ti-situation').value='';$('sakura-ti-result').hidden=true;status('');return}
